...
class StatisticsDatabaseStorage implements StatisticsStorageInterface {
    ...
    try {
      return (bool) $this->connection
        ...
        ->fields([
          ...
          'timestamp_YmdHis' => date('Y-m-d H:i:s',$this->requestStack->getCurrentRequest()->server->get('REQUEST_TIME')),
        ])
  ...
  public function fetchView(EntityTypeInterface $entity_type, $id) {
    try {
      $view = $this->connection
        ...
        ->fields('c', ['totalcount', 'daycount', 'timestamp', 'timestamp_YmdHis'])
        ->condition($entity_type->getKey('id'), $id)
        ->execute()
        ->fetchObject();
      if ($view) {
        return new StatisticsViewsResult($view->totalcount, $view->daycount, $view->timestamp, $view->timestamp_YmdHis);
      }
    }
    catch (\Exception $e) {
  ...
  public function fetchAll(EntityTypeInterface $entity_type, $order = 'totalcount', $limit = 5) {
    assert(in_array($order, ['totalcount', 'daycount', 'timestamp', 'timestamp_YmdHis']), "Invalid order argument.");
    try {
    ...
    if (!$this->connection->schema()->tableExists($table)) {
      $schema = [
        ...
        'fields' => [
         ...
         'timestamp_YmdHis' => [
            'description' => "The most recent date the {{$entity_type_id}} has been viewed.",
            'type' => 'varchar',
            'mysql_type' => 'datetime',
            'not null' => FALSE,
          ],
